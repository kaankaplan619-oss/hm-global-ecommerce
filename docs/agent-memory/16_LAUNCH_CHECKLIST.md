...
18. ⏳ **Revue externe (dev prestataire emails/OVH)** : accès = URL du site + compte test auto-créé + GitHub **lecture seule** si avis code. JAMAIS : membre du team Vercel (les variables d'env y sont lisibles : Stripe LIVE, Supabase service role, Printful), admin du site (données clients), clés API, `.env`. Pour qu'il voie les correctifs du 2026-06-12 : pousser `codex/hermes-os-cockpit-v1` → URL de preview Vercel (GO Kaan requis).
